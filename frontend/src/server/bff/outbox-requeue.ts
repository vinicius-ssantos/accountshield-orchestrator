...
// session.ts, require-session.ts, and every other mutation module: the real architecture boundary
...
import { AccountShieldOutboxRequeueClient, parseRequeueInput, type OutboxRequeueService } from "./outbox-requeue-core";
 ...
 */
export function createOutboxRequeueClient(
  request: Request,
  source: Readonly<Record<string, string | undefined>> = process.env,
): AccountShieldOutboxRequeueClient {
  const environment = readFrontendEnvironment(source, "runtime");
  if (environment.dataSource !== "live" || !environment.apiUrl) {
    throw new BffError("UPSTREAM_UNAVAILABLE", 503, "Live outbox requeue is not configured.", true);
  }

  const { backendToken } = requireOperatorSession(request, source);

  return new AccountShieldOutboxRequeueClient({
...
export async function handleRequeueRequest(
  request: Request,
  service?: OutboxRequeueService,
  telemetrySink?: BffTelemetrySink,
  source: Readonly<Record<string, string | undefined>> = process.env,
): Promise<Response> {
  const correlationId = resolveCorrelationId(request.headers.get("x-correlation-id"));
  const telemetry = startBffTelemetry({ useCase: "outbox_requeue", correlationId, sink: telemetrySink });
  ...
  try {
    ...
    const resolvedService = service ?? createOutboxRequeueClient(request, source);
    const result = await resolvedService.requeue(parseRequeueInput(body), correlationId, request.signal);
