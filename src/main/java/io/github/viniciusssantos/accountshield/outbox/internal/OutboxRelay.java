...
public class OutboxRelay {
    ...
    private void dispatchSingle(ClaimedOutboxEvent event) {
        try {
            ...
            claimStore.markPublished(event.id(), clock.instant());
            increment("published");
        } catch (Exception ex) {
            int newAttemptCount = event.attemptCount() + 1;
            String error = boundError(ex);
            if (newAttemptCount >= maxAttempts) {
                claimStore.markDeadLettered(event.id(), newAttemptCount, error, clock.instant());
                increment("dead_lettered");
                log.warn(
                ...
                claimStore.markFailedWithBackoff(event.id(), newAttemptCount, error, nextAttemptAt);
                increment("failed");
                log.warn(
