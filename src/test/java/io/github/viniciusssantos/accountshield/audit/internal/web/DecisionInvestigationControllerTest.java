...
class DecisionInvestigationControllerTest {
    ...
    void returnsMinimizedPageWithNoStore() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/operator/decisions/search")
                ...
                .andExpect(jsonPath("$.decisions[0].correlationId")
                        .value("corr-safe-123"))
    ...
    void doesNotLeakDatabaseFailureDetails() throws Exception {
        when(query.search(any())).thenThrow(new DataRetrievalFailureException(
                "account secret and internal SQL must not leak"));
        ...
        mockMvc.perform(post("/api/v1/operator/decisions/search")
                ...
                .andExpect(content().string(Matchers.not(
                        Matchers.containsString("account secret"))))
                .andExpect(content().string(Matchers.not(
                        Matchers.containsString("internal SQL"))));
    }
}
