...
| Authorized policy directory and investigation read API | **Implemented** | `SECURITY_OPERATOR` can list policy keys with lifecycle/rollout summary, or submit a policy key and receive its full version history (governance, analyzer diagnostics), routing scope, active rollout, and impact analysis with explicit `AVAILABLE`/`NOT_APPLICABLE`/`UNAVAILABLE` availability — without `POLICY_ADMIN` or `SIMULATION_ANALYST` privileges | ADR 0044; [#181](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/181); frontend consumer [#73](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/73) |
...
| Authorized decision investigation read API | **Implemented** | `SECURITY_OPERATOR` can search recent decisions or an exact correlation ID through a body-based, `no-store`, keyset-paginated endpoint that returns a minimized projection without raw account, fingerprint, context, reason-detail, challenge, or recovery fields | ADR 0040; [#134](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/134); frontend consumer [#69](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/69) |
| Authorized decision investigation timeline API | **Implemented** | `SECURITY_OPERATOR` can submit an opaque decision reference in a JSON body and receive masked subject context, ordered reasons, bounded provenance, payload-free challenge/recovery/outbox summaries, explicit section availability, and a deterministic `no-store` timeline | ADR 0041; [#171](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/171); frontend consumer [#70](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/70) |
...
| Authorized decision replay comparison read API | **Implemented** | `SECURITY_OPERATOR` can submit an opaque decision reference in a JSON body and receive a masked, side-effect-free replay comparison (original vs. replayed outcome/score/band/reasons, historical policy/algorithm/schema/catalog/engine versions, human-readable mismatches) without simulation-analyst privileges; explicit `no-store` and stable Problem Details for unresolved references and unresolvable historical versions | ADR 0043; [#178](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/178); frontend consumer [#72](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/72) |
...
| Authorized recovery investigation read API | **Implemented** | `SECURITY_OPERATOR` can search a body-based, keyset-paginated, `no-store` recovery queue and retrieve one privacy-minimized detail with masked subject, classification/version, persisted timing, derived review state and explicit challenge availability; command services and sensitive challenge/reviewer/account fields are excluded | ADR 0042; [#174](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/174); frontend consumer [#71](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/71) |
...
| Authorized outbox delivery health and search read API | **Implemented** | `SECURITY_OPERATOR` can submit a body-based, `no-store` search (status(es), event type, occurred-time window, attempt-count range, keyset cursor) and receive aggregate health (pending/retrying/in-progress/dead-lettered counts, oldest-pending age, windowed recent dead-letter/publish counts) plus a filtered, masked event page; dead-letter diagnostics expose only a bounded failure category (exception simple class name captured at write time), never the raw exception message; `nextAttemptAt` is never returned for terminal-state rows | ADR 0045; [#184](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/184); frontend consumer [#74](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/74) |
...
| API authentication and RBAC | **Partial** | A stateless JWT resource server and backend-enforced roles protect sensitive APIs, including decision and recovery investigation read contracts; remaining issue #19 acceptance and documentation alignment still need closure | ADR 0011; ADR 0040; ADR 0041; ADR 0042; [#19](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/19) |
| Demo operator credential login | **Implemented** | `POST /auth/session-tokens` verifies a small, fixed set of config-seeded, bcrypt-hashed demo personas and issues a JWT through the existing local signer, with a companion refresh endpoint; not a real identity provider | ADR 0046, [#189](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/189) |
...
| Operator console | **Partial** | Next.js/BFF operator console is part of `main`: read-only investigation (decisions, recoveries, policies, replay, outbox) plus authenticated operator mutations (recovery review with step-up, policy lifecycle/rollout, dead-letter requeue, evidence export) gated behind a BFF session; backend authorization remains authoritative. Remaining portfolio polish (reproducible demo walkthrough, deployment/rollback/session-revocation docs) tracked under the epic | [#41](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/41), [frontend architecture](../frontend/architecture.md) |
