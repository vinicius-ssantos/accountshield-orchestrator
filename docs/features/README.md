...
| Deterministic policy replay | **Partial** | Replay re-runs the recorded risk algorithm via a versioned registry and reports field-level mismatches (score, band, reasons, outcome) without any operational side effect (ADR 0019); broader provenance (schema/reason-catalog/decision-engine versions, commit SHA, input hash) is not yet captured | [#43](https://github.com/vinicius-ssantos/accountshield-orchestrator/issues/43) |
